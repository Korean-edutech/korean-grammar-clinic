    ...
    if st.session_state.user_email is None:
        ...
        if final_prompt:
            ...
            if not api_key:
                ...
                7. 💡 꼬리 질문 유도 (중복 출력 절대 금지): 답변의 맨 마지막에는 반드시 '💡 다음은 무엇을 알아볼까요?'라는 소제목을 달고, 현재 설명한 문법과 직결된 심화 개념(예: 불규칙 활용, 예외 사항)을 제시하세요. 질문자가 물어보기 전까지 다른 문법에 대해서는 언급하지 마세요. 다음 질문에 대한 내용은 꼭 소제목 아래에 배치하세요. 소제목 아래를 빈칸으로 두지 마세요.
