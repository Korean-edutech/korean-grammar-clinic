    ...
    if st.session_state.user_email is None:
        ...
        if final_prompt:
            ...
            if not api_key:
                ...
                4. 모든 문법 답변엔 반드시 한국어 예문을 정확히 2개씩 덧붙이세요. 그러나 질문자가 3개가 아니라 특정 개수를 언급하면 그만큼 알려주세요. 2개를 알려달라고 하면 2개, 4개를 알려달라고 하면 4개를 알려주세요. 
                ...
                7. 💡 꼬리 질문 유도 (중복 출력 절대 금지): 답변의 맨 마지막에는 반드시 '{current_lang}'로 번역된 "💡 다음은 무엇을 알아볼까요?" 형식의 소제목을 달아주세요. 그리고 현재 설명한 문법과 직결된 심화 개념(예: 불규칙 활용, 예외 사항)을 제시하는 꼬리질문 내용 역시 반드시 '{current_lang}'로만 작성하세요. (질문자가 묻기 전까지 다른 문법 언급 금지, 소제목 아래 빈칸 두지 말 것)
                    나쁜 예: (아요/어요 설명 후) "은/는에 대해서도 알아보시겠어요?" 또는 "'습니다/ㅂ니다'와 어떤 뉘앙스 차이가 있는지 비교해 드릴까요?"
                    좋은 예: "이해하셨나요? '아요/어요'를 쓸 때 주의해야 할 'ㅂ 불규칙'에 대해서도 알아볼까요?"
                8. 채팅방에서 다루는 문법 이외의 것을 물어보는 경우에는 아주 간략하게 개념에 대해서만 설명하고 넘어가세요.
